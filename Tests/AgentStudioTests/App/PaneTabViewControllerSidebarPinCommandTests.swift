...
struct PaneTabViewControllerSidebarPinCommandTests {
    ...
    func targetedRepositoryPinCommandsMutateOnlyRepositoryPinState() async {
    ...
    func targetedPanePinCommandsMutateOnlySelectedPane() async {
        let harness = makeHarness()
        defer { try? FileManager.default.removeItem(at: harness.tempDir) }
        await withWorkspaceCommandHarness(harness) {
    ...
    func pinCommandsRejectWrongTargetKind() async {
        let harness = makeHarness()
        defer { try? FileManager.default.removeItem(at: harness.tempDir) }
        await withWorkspaceCommandHarness(harness) {
            let (repo, _) = makeRepoAndWorktree(harness.store, root: harness.tempDir)
