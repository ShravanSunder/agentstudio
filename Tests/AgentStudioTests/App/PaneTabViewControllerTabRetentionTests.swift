...
struct PaneTabViewControllerTabRetentionTests {
    ...
    private func withHarness<T>(_ body: @MainActor (Harness) throws -> T) rethrows -> T {
    ...
    private func withHarness<T>(_ body: @MainActor (Harness) async throws -> T) async rethrows -> T {
        ...
        try await withHarness { harness in
            ...
            #expect(
                atom(\.workspaceFocusOwner).owner
                    == .drawerPane(parentPaneId: parentPane.id, paneId: drawerPane.id))
