...
struct AppDelegateLifecycleRoutingTests {
    ...
    @Test("post-boot synchronization seeds inactive application state")
    func postBootSynchronizationSeedsInactiveApplicationState() {
        let delegate = AppDelegate()
        let appLifecycleStore = AppLifecycleAtom()
        let windowLifecycleStore = WindowLifecycleAtom()
        delegate.appLifecycleStore = appLifecycleStore
        ...
        delegate.synchronizeApplicationLifecycleStateAfterWorkspaceBoot(isApplicationActive: false)
