...
final class SplitTreeTests: XCTestCase {
    ...
    func test_codable_emptyTree_roundTrips() throws {
        // Arrange
        let tree = TestSplitTree()
        ...
        let decoded = try decoder.decode(TestSplitTree.self, from: data)
    ...
    func test_codable_singleView_roundTrips() throws {
        // Arrange
        let view = MockTerminalView(name: "solo")
        let tree = TestSplitTree(view: view)
        let encoder = JSONEncoder()
        let decoder = JSONDecoder()
        ...
        XCTAssertEqual(decoded.allViews.count, 1)
        XCTAssertEqual(decoded.allViews[0].id, view.id)
        XCTAssertEqual(decoded.allViews[0].name, "solo")
    }

    func test_codable_splitTree_roundTrips() throws {
        // Arrange
        let viewA = MockTerminalView(name: "A")
        let viewB = MockTerminalView(name: "B")
        let tree = try TestSplitTree(view: viewA)
        ...
        XCTAssertEqual(decoded.allViews.count, 2)
        XCTAssertEqual(decoded.allViews[0].id, viewA.id)
        XCTAssertEqual(decoded.allViews[0].name, "A")
        XCTAssertEqual(decoded.allViews[1].id, viewB.id)
        XCTAssertEqual(decoded.allViews[1].name, "B")
    ...
    func test_codable_deepTree_roundTrips() throws {
        ...
        // Assert
        XCTAssertEqual(decoded.allViews.count, 3)
        XCTAssertEqual(decoded.allViews[0].name, "A")
        XCTAssertEqual(decoded.allViews[1].name, "B")
        XCTAssertEqual(decoded.allViews[2].name, "C")

        guard case .split(let rootSplit) = decoded.root else {
            XCTFail("Expected root split")
            return
        }
        XCTAssertEqual(rootSplit.direction, .horizontal)
        XCTAssertEqual(rootSplit.ratio, 0.4, accuracy: 0.001)

        guard case .split(let nestedSplit) = rootSplit.right else {
            XCTFail("Expected nested split")
            return
        }
        XCTAssertEqual(nestedSplit.direction, .vertical)
        XCTAssertEqual(nestedSplit.ratio, 0.5, accuracy: 0.001)
    }
}
