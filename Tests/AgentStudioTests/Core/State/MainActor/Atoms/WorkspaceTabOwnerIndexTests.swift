...
struct WorkspaceTabOwnerIndexTests {
    ...
    @Test("Exact tab graph insertion updates all maintained indexes")
    func exactTabGraphInsertionUpdatesMaintainedIndexes() {
        // Arrange
        let atom = WorkspaceTabGraphAtom()
        let existing = makeTabGraphState()
        let inserted = makeTabGraphState()
        atom.replaceTabStates([existing])

        // Act
        atom.insertTabState(inserted, at: 0)

        // Assert
        #expect(atom.tabCount == 2)
        #expect(atom.containsTab(existing.tabId))
        #expect(atom.containsTab(inserted.tabId))
        #expect(atom.tabIndex(for: inserted.tabId) == 0)
        #expect(atom.tabIndex(for: existing.tabId) == 1)
        #expect(atom.tabID(containingPane: inserted.allPaneIds[1]) == inserted.tabId)
        #expect(atom.tabID(containingArrangement: inserted.arrangements[1].id) == inserted.tabId)
        #expect(atom.tabID(containingArrangement: existing.arrangements[0].id) == existing.tabId)
    }

    @Test("Cursor presence queries distinguish absent keys from explicit empty selections")
    func cursorPresenceQueriesDistinguishAbsentKeysFromEmptySelections() {
        // Arrange
        ...
        // Assert
        #expect(atom.hasActiveArrangementCursor(tabID: tabID))
        #expect(atom.hasPaneCursor(arrangementID: arrangementID))
        #expect(atom.hasDrawerCursor(drawerKey))
        #expect(!atom.hasActiveArrangementCursor(tabID: UUIDv7.generate()))
        #expect(!atom.hasPaneCursor(arrangementID: UUIDv7.generate()))
        #expect(
            !atom.hasDrawerCursor(
    ...
    @Test("Cursor presence queries follow exact insertion")
    func cursorPresenceQueriesFollowExactInsertion() {
        // Arrange
        let atom = WorkspaceArrangementCursorAtom()
        let tabID = UUIDv7.generate()
        let arrangementID = UUIDv7.generate()
        ...
        // Act
        atom.insertActiveArrangementId(arrangementID, forTab: tabID)
        atom.insertPaneCursor(.init(activePaneId: nil), forArrangement: arrangementID)
        atom.insertDrawerCursor(.init(activeChildId: nil), for: drawerKey)
