...
@MainActor
final class PaneRemovalCascadeTests: XCTestCase {
    ...
    func test_removePane_removesFromDefaultAndCustomArrangements() {
        ...
        XCTAssertNil(store.pane(paneIds[1]))
        ...
        XCTAssertFalse(defaultArr.visiblePaneIds.contains(paneIds[1]))
        ...
        XCTAssertFalse(customArr.visiblePaneIds.contains(paneIds[1]))
    }
    ...
    func test_removePane_resetsActivePaneId_ifRemoved() {
        ...
        let updatedTab = store.tabs.first { $0.id == tab.id }!
        XCTAssertNotNil(updatedTab.activePaneId)
        XCTAssertNotEqual(updatedTab.activePaneId, paneIds[1])
    }
    ...
    func test_removePane_clearsZoom_ifZoomedPaneRemoved() {
        ...
        XCTAssertEqual(store.tab(tab.id)!.zoomedPaneId, paneIds[0])
    ...
    func test_removePane_removesTabWhenDefaultArrangementEmpty() {
        ...
        XCTAssertNil(store.tabs.first { $0.id == tab.id })
    }
    ...
    func test_removePane_removesFromTabPanesList() {
        ...
        let updatedTab = store.tabs.first { $0.id == tab.id }!
        XCTAssertFalse(updatedTab.panes.contains(paneIds[1]))
    ...
    func test_removePaneFromLayout_resetsActivePaneId() {
        ...
        if let tab = updatedTab {
            XCTAssertNotEqual(tab.activePaneId, paneIds[1])
        }
    }
    ...
    func test_removePaneFromLayout_clearsZoom() {
        ...
        let updatedTab = store.tab(tab.id)!
        XCTAssertNil(updatedTab.zoomedPaneId)
    }
    ...
    func test_backgroundedPane_notInTabLayouts_removedByPurge() {
        ...
        XCTAssertNotNil(store.pane(targetPaneId))
        XCTAssertEqual(store.pane(targetPaneId)!.residency, .backgrounded)

        // Should not be in tab's active layout
        let updatedTab = store.tab(tab.id)!
        XCTAssertFalse(updatedTab.paneIds.contains(targetPaneId))

        // Purge should remove it
        store.purgeOrphanedPane(targetPaneId)
        XCTAssertNil(store.pane(targetPaneId))
    }
    ...
    func test_reactivatePane_restoresIntoLayout() {
        ...
        store.backgroundPane(targetPaneId)
        XCTAssertFalse(store.tab(tab.id)!.paneIds.contains(targetPaneId))
        ...
        XCTAssertTrue(store.tab(tab.id)!.paneIds.contains(targetPaneId))
        XCTAssertEqual(store.pane(targetPaneId)!.residency, .active)
    }
    ...
    func test_removePane_cleanedFromMultipleTabs() {
        ...
        XCTAssertFalse(updatedTab1.paneIds.contains(pane2.id))
        XCTAssertFalse(updatedTab2.paneIds.contains(pane2.id))
    }
}
