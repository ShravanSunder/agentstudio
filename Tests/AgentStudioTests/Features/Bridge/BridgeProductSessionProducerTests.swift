...
struct BridgeProductSessionProducerTests {
    ...
    @Test("ordinary admission reports capacity absence without queue or sequence mutation")
    func ordinaryAdmissionReportsNonmutatingCapacityAbsence() async throws {
        // Arrange
        let request = try producerRegistryMetadataStreamRequest()
        let openingFrame = try producerRegistryMetadataOpeningFrame(for: request, sequence: 0)
        let firstFrame = try producerRegistryMetadataProgressFrame(
            for: request,
            sequence: 1,
            identitySuffix: "capacity-first"
        )
        let secondFrame = try producerRegistryMetadataProgressFrame(
            for: request,
            sequence: 2,
            identitySuffix: "capacity-second"
        )
        let waitingFrame = try producerRegistryMetadataProgressFrame(
            for: request,
            sequence: 3,
            identitySuffix: "capacity-waiting"
        )
        let maximumEncodedFrameByteCount = try #require(
            [openingFrame, firstFrame, secondFrame, waitingFrame]
                .map { try $0.encode().count }
                .max()
        )
        let limits = try BridgeProductProducerQueueLimits(
            maximumQueuedFrameCount: 3,
        ...
        let lease = try #require(registration.lease)
        _ = await operation.waitUntilStarted()
        _ = try await registry.enqueueRequiredOpeningFrame(for: lease) { _ in openingFrame }
        #expect(await registry.consumeNextFrame(for: lease)?.requiredOpening == true)
        _ = try await registry.enqueueNonterminalFrame(
            for: lease,
            build: { _ in firstFrame },
        ...
        let beforeAttempt = await registry.snapshot()

        // Act
        let result = try await registry.attemptNonterminalFrameAdmission(
            for: lease,
            build: { _ in waitingFrame }
        )
        let afterAttempt = await registry.snapshot()

        // Assert
        #expect(result == .capacityUnavailable)
        #expect(afterAttempt == beforeAttempt)
        #expect(afterAttempt.nextMetadataStreamSequence == 3)
        #expect(afterAttempt.queuedFrameCount == 2)
        try await closeAllProducerRegistryProducers(in: registry)
    }
