...
struct RepoExplorerTableRowHeightInvalidationTests {
    @Test("native context menu remains independent while its row height updates")
    func nativeContextMenuRemainsIndependentWhileRowHeightUpdates() throws {
        ...
        let menu = try #require(materializer.makeContextMenu(forRowID: fixture.rowID))
        let menuStructure = menu.items.map {
            "\($0.title):\($0.submenu?.items.map(\.title).joined(separator: ",") ?? "")"
        }
        ...
        #expect(updatedCell.currentBindingIdentity?.visibleGeneration == 2)
