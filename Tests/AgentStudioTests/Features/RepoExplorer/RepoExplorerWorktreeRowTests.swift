...
struct RepoExplorerWorktreeRowTests {
    ...
    func resolvedUnavailablePullRequestStateRendersNoGlyphAndNoChip() {
        ...
        // honesty threshold) must never render as still-pending, but a real
        // positive PR count must still stay visible.
    ...
    @Test("a stale positive PR count still renders once the repo resolves unavailable")
    func stalePositivePullRequestCountStillRendersOnceUnavailable() throws {
        ...
        // terminally unavailable (no remote, or repeated failures past the honesty threshold).
        // The contract now keeps the count chip visible so the row does not lose a real PR signal
        // just because loading is no longer pending.
