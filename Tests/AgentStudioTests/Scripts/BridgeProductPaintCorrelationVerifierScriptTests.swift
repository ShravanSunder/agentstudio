...
struct BridgeProductPaintCorrelationVerifierScriptTests {
    ...
    func verifierDryRunStatesCompleteProofContract() throws {
        ...
        #expect(result.stdout.contains("including exact Review/File identity chains"))
        #expect(result.stdout.contains("reload replay, and worker replacement"))
        #expect(result.stdout.contains("requires exactly one selected Review source match"))
    ...
    func verifierRequiresExactOneScopedProofAndLiveIdentity() throws {
        ...
        #expect(source.contains("review_identity_chain_matched"))
        #expect(source.contains("file_identity_chain_matched"))
        #expect(source.contains("reload_replay_succeeded"))
        #expect(source.contains("worker_replacement_observed"))
        #expect(source.contains("normalized_int(record.get(field)) == 1"))
