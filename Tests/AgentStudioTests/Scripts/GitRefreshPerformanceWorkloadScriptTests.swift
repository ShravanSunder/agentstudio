...
struct GitRefreshPerformanceWorkloadScriptTests {
    ...
    func workloadProofScriptHasStableSafetyContractAndBashSyntax() throws {
        ...
        #expect(source.contains("current_trace_jsonl_has_event \"$event_name\""))
        #expect(source.contains("current_trace_jsonl_has_command_bar_filter"))
        #expect(source.contains("performance.coordinator.write \\"))
        #expect(source.contains("echo \"$event_name victoria_count=$victoria_count jsonl_count=$jsonl_count\""))
