...
struct ArchitectureSwiftLintRulesTests {
    ...
    func architectureLintWiringUsesStockSwiftLintAndLocalTool() throws {
        ...
        #expect(lintScript.contains("scoped_swift_paths=()"))
        #expect(lintScript.contains("swift-format lint \"${scoped_swift_paths[@]}\""))
        #expect(lintScript.contains("swiftlint lint --strict \"${scoped_swift_paths[@]}\""))
