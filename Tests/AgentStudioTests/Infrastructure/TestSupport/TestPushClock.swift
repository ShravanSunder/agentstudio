...
struct TestPushClock: Clock {
        ...
        let _: Void = try await withTaskCancellationHandler(
            operation: {
                try await withUnsafeThrowingContinuation { (continuation: UnsafeContinuation<Void, Error>) in
                    ...
                    let shouldResume = state.withCriticalRegion { state in
                        if Task.isCancelled {
                            shouldThrowCancellation = true
                            return true
                        }
                        if deadline.nanoseconds <= state.now {
                            return true
                        }
                        ...
                        resumedWaiters = Self.dequeueSatisfiedPendingSleepWaiters(state: &state)
                        return false
                    }
    ...
    func advance(to instant: Instant) {
        ...
        state.withCriticalRegion { state in
            let nextNow = max(state.now, instant.nanoseconds)
            state.now = nextNow
            let remaining = state.pending.filter { $0.deadline > nextNow }
            let resumed = state.pending.filter { $0.deadline <= nextNow }
    ...
    private func waitForPendingSleepCount(matching condition: PendingSleepWaiterCondition) async {
        let shouldResumeImmediately = state.withCriticalRegion { state in
            condition.isSatisfied(by: state)
        }
        ...
        await withUnsafeContinuation { (continuation: UnsafeContinuation<Void, Never>) in
            let shouldResume = state.withCriticalRegion { state in
                if condition.isSatisfied(by: state) {
                    return true
                }

                state.pendingSleepWaiters.append(
    ...
    private func cancel(_ generation: Int) {
        var resumedWaiters: [UnsafeContinuation<Void, Never>] = []
        let continuation = state.withCriticalRegion { state -> UnsafeContinuation<Void, Error>? in
            guard let index = state.pending.firstIndex(where: { $0.generation == generation }) else {
                return nil
            }
            let continuation = state.pending.remove(at: index).continuation
            resumedWaiters = Self.dequeueSatisfiedPendingSleepWaiters(state: &state)
            return continuation
        }
        continuation?.resume(throwing: CancellationError())
