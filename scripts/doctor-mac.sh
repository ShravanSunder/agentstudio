...
vendor_worktree_helper="$project_root/scripts/vendor-worktree.sh"
...
if [[ ! -x "$vendor_worktree_helper" ]]; then
  report_error "vendor worktree helper is unavailable: $vendor_worktree_helper"
elif vendor_role="$("$vendor_worktree_helper" role 2>&1)"; then
  report_ok "vendor worktree role: $vendor_role"
else
  report_error "vendor worktree role detection failed: $vendor_role"
fi

if [[ "$vendor_role" == "primary" ]]; then
  if vendor_verify_output="$("$vendor_worktree_helper" verify 2>&1)"; then
    report_ok "vendor inputs are ready"
  else
    report_warn "primary vendor inputs are not prepared yet; mise run setup will prepare them"
  fi
elif [[ -x "$vendor_worktree_helper" && -n "$vendor_role" ]]; then
  if vendor_verify_output="$("$vendor_worktree_helper" verify 2>&1)"; then
    report_ok "vendor inputs are ready"
  else
    report_error "$vendor_verify_output"
  fi
fi

if [[ "$vendor_role" == "primary" || "$vendor_role" == "local" ]]; then
  ...
  problem_vars=(CC CXX CFLAGS CXXFLAGS CPPFLAGS LDFLAGS CPATH LIBRARY_PATH SDKROOT DEVELOPER_DIR MACOSX_DEPLOYMENT_TARGET)
  present_vars=()
  ...
  report_ok "non-producer worktree does not require local submodules or Zig"
fi
