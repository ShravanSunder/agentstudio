...
if [ "$state_status" = "launch_failed" ] || [ "$state_status" = "already_running" ]; then
  if [ -n "$state_reason" ]; then
    echo "AgentStudio stable observability did not start: $state_status ($state_reason)" >&2
  else
    echo "AgentStudio stable observability did not start: $state_status" >&2
  fi
...
EXPECTED_APP="${AGENTSTUDIO_EXPECTED_STABLE_APP:-$state_app}"
if [ -z "$MARKER" ]; then
  echo "missing AgentStudio stable observability marker; run mise run run-stable-preferences-observability first" >&2
  exit 1
fi
if [ -z "$EXPECTED_APP" ]; then
  echo "missing AGENTSTUDIO_EXPECTED_STABLE_APP; stable proof must bind verification to the exact launched app" >&2
  echo "state file: $STATE_FILE" >&2
  exit 1
fi
if [ "$state_preferences_mode" != "honor_preferences" ]; then
  echo "stable observability state is not preference-honoring" >&2
  echo "state file: $STATE_FILE" >&2
  exit 1
fi
...
process_executable_path() {
  ...
  if ! txt_output="$("$LSOF_BIN" -a -p "$pid" -d txt -Fn 2>/dev/null)"; then
    echo "unable to inspect AgentStudio stable PID $pid with $LSOF_BIN" >&2
    return 1
  fi
...
actual_app="$(realpath_or_empty "$state_app")"
expected_app="$(realpath_or_empty "$EXPECTED_APP")"
if [ -z "$actual_app" ] || [ "$actual_app" != "$expected_app" ]; then
  echo "AgentStudio stable observability app mismatch" >&2
...
if [ "$state_status" != "running" ]; then
  echo "AgentStudio stable observability state is not running: ${state_status:-<missing>}" >&2
  echo "state file: $STATE_FILE" >&2
  exit 1
fi
case "$state_pid" in
  ''|*[!0-9]*)
    echo "AgentStudio stable observability state missing numeric PID" >&2
    echo "state file: $STATE_FILE" >&2
    exit 1
    ;;
esac
if ! kill -0 "$state_pid" >/dev/null 2>&1; then
  echo "AgentStudio stable observability PID is not running: $state_pid" >&2
  echo "state file: $STATE_FILE" >&2
  exit 1
fi
process_executable="$(process_executable_path "$state_pid")"
if [ -z "$process_executable" ] ||
  [ "$(bundle_release_channel_for_executable "$process_executable")" != "stable" ]; then
  echo "AgentStudio stable observability PID does not resolve to a stable app: $state_pid" >&2
  echo "state file: $STATE_FILE" >&2
  exit 1
fi
...
if [ -z "$process_app" ] || [ "$process_app" != "$expected_app" ]; then
  echo "AgentStudio stable observability PID app mismatch" >&2
  echo "expected: $expected_app" >&2
...
stream_query="{service.name=\"AgentStudio\",dev.release.channel=\"stable\"}"
...
if [ -z "$positive_response" ]; then
  echo "no AgentStudio stable records found in VictoriaLogs for query window $QUERY_START..$QUERY_END" >&2
  exit 1
fi
...
echo "stable preferences observability ok:"
sed -n '1,5p' <<<"$preferences_response"
