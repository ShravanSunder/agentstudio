...
| 5 | `tmp-screenshots/contract-final/17-item5-all-panes.png`; `tmp-screenshots/contract-final/17-item5-by-tab.png` | PASS — a terminal OSC notification traversed the live inbox pipeline with body `Content-bearing inbox proof`; L2 renders that body in All Panes and By Tab (AX confirms the same content), replacing the generic fallback. |
...
| 13 | `tmp-screenshots/contract-final/18-empty-by-repo.png`; `tmp-screenshots/contract-final/18-empty-all-panes.png`; `tmp-screenshots/contract-final/18-empty-by-tab.png`; `Tests/AgentStudioTests/Features/RepoExplorer/RepoExplorerPaneProjectionTests.swift` | PASS — a genuine fresh workspace with an empty query renders the centered true-empty state per mode: `No repositories`, `No panes`, and `No tabs`; projection tests independently assert the same mode-specific states. |
...
| 19 | `tmp-screenshots/contract-final/20-item19-bare-stale-dot.png`, `tmp-screenshots/contract-final/20-item19-bare-stale-dot-sidebar.png` | PASS — measured in the built By Repo app: the dirty `agent-vm.fix-tools` row’s `+696 -19` value chip is followed by a separate small hollow-circle SF Symbol at the same chip-line height, with quiet secondary paint and no pill, background, or border. Refresh animation remains deferred until a real keyed lifecycle fact exists and will animate this same bare glyph in place. |
