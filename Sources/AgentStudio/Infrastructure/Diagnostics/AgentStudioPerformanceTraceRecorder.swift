...
package final class AgentStudioPerformanceTraceRecorder: @unchecked Sendable {
    ...
    package func recordRendererLifecycle(
        ...
        let attributes = lock.withLock { () -> [String: AgentStudioTraceValue] in
            var createdDelta = 0
            var releasedDelta = 0
            switch action {
            case .created:
                rendererLifecycleState.createdTotal &+= 1
                createdDelta = 1
            case .released:
                rendererLifecycleState.releasedTotal &+= 1
                releasedDelta = 1
            ...
            var attributes = Self.rendererLifecycleAttributes(
                action: action,
                snapshot: rendererLifecycleState.snapshot,
    ...
    package func recordRendererFreed() {
        let attributes = lock.withLock { () -> [String: AgentStudioTraceValue] in
            rendererLifecycleState.freedTotal &+= 1
            rendererLifecycleState.sampleSequence &+= 1
    ...
    package func recordRendererVisibilityReconciliation(
        ...
        let attributes = lock.withLock { () -> [String: AgentStudioTraceValue]? in
            guard applied + missing > 0 else {
                rendererLifecycleState.reconciliationEqualSinceLastEmit &+= equal
                return nil
            }
            let equalSinceLastEmit = rendererLifecycleState.reconciliationEqualSinceLastEmit
            rendererLifecycleState.reconciliationEqualSinceLastEmit = 0
            rendererLifecycleState.sampleSequence &+= 1
            var attributes = Self.rendererLifecycleAttributes(
                action: .reconciled,
                snapshot: rendererLifecycleState.snapshot,
