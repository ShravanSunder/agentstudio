...
package enum RepositoryLifecycleReconciliation {
    @concurrent nonisolated package static func prepare(
        ...
        for group in groups {
            ...
            let paths = ([familyPath] + group.linkedWorktreePaths).filter { positivePaths.contains($0) }
            for path in paths {
    ...
    {
        var evidenceByPath: [URL: RepoScanner.ResolvedGitEntry] = [:]
        for entry in observation.entries {
            let path = entry.path.standardizedFileURL
            if let previous = evidenceByPath[path],
                previous.repositoryKey != entry.repositoryKey || previous.kind != entry.kind
            {
                return .duplicateWorktreeStableKey(StableKey.fromPath(path))
            }
