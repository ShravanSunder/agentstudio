...
extension AppDelegate {
    func applicationDidBecomeActive(_ notification: Notification) {
        guard let applicationLifecycleMonitor else {
            appDelegateLifecycleLogger.warning(
                "Skipping applicationDidBecomeActive before lifecycle monitor is ready"
            )
    ...
    func applicationDidResignActive(_ notification: Notification) {
        guard let applicationLifecycleMonitor else {
            appDelegateLifecycleLogger.warning(
                "Skipping applicationDidResignActive before lifecycle monitor is ready"
            )
    ...
    func applicationWillTerminate(_ notification: Notification) {
        ...
        guard let applicationLifecycleMonitor else {
            appDelegateLifecycleLogger.warning(
                "Skipping applicationWillTerminate before lifecycle monitor is ready"
            )
