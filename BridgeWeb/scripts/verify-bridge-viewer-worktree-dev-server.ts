...
import type { OwnedBridgeDevelopmentServer } from './dev-server/bridge-development-server-process.js';
...
type BridgeDevelopmentServerProcessModule = Pick<
	typeof import('./dev-server/bridge-development-server-process.js'),
	'runAllOwnedCleanupOperations' | 'startOwnedBridgeDevelopmentServer'
>;
...
if (performanceOnlyMode || startupOnlyMode) {
...
async function runSelfHostedBridgeViewerPerformanceVerifier(): Promise<void> {
	...
	let bridgeDevelopmentServerProcessModule: BridgeDevelopmentServerProcessModule | null = null;
	let bridgeDevelopmentServer: OwnedBridgeDevelopmentServer | null = null;
	let moduleLoader: ViteDevServer | null = null;
	let viteServer: ViteDevServer | null = null;
	try {
		moduleLoader = await createViteServer({
			configFile: false,
			optimizeDeps: { noDiscovery: true },
			server: { middlewareMode: true },
		});
		bridgeDevelopmentServerProcessModule =
			await loadBridgeDevelopmentServerProcessModule(moduleLoader);
		await moduleLoader.close();
		moduleLoader = null;
		backendDataRootPath = await mkdtemp(join(tmpdir(), 'agentstudio-bridge-performance-'));
		const backendPort = await reserveLoopbackPort();
		bridgeDevelopmentServer =
			await bridgeDevelopmentServerProcessModule.startOwnedBridgeDevelopmentServer({
		...
		try {
			const cleanupOperations = [
				{
					name: 'performance Vite module loader',
					run: async (): Promise<void> => await moduleLoader?.close(),
				},
				{
			...
			if (bridgeDevelopmentServerProcessModule === null) {
				for (const operation of cleanupOperations) {
					// oxlint-disable-next-line no-await-in-loop -- Failure cleanup is intentionally exhaustive and ordered.
					await operation.run();
				}
			} else {
				await bridgeDevelopmentServerProcessModule.runAllOwnedCleanupOperations({
...
async function loadBridgeDevelopmentServerProcessModule(
	moduleLoader: ViteDevServer,
): Promise<BridgeDevelopmentServerProcessModule> {
	const loadedModule: unknown = await moduleLoader.ssrLoadModule(
		'/scripts/dev-server/bridge-development-server-process.ts',
	);
	if (
		typeof loadedModule !== 'object' ||
		loadedModule === null ||
		!('runAllOwnedCleanupOperations' in loadedModule) ||
		typeof loadedModule.runAllOwnedCleanupOperations !== 'function' ||
		!('startOwnedBridgeDevelopmentServer' in loadedModule) ||
		typeof loadedModule.startOwnedBridgeDevelopmentServer !== 'function'
	) {
		throw new Error('Expected the owned Bridge development server process module.');
	}
	return loadedModule as BridgeDevelopmentServerProcessModule;
}
