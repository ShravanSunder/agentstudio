...
export const worktreeFileDescriptorSchema = z
	.object({
		...
		isBinary: z.boolean(),
		unavailableReason: z.enum(['unreadable', 'unsupported_encoding']).nullable(),
	...
	.superRefine((descriptor, context): void => {
		...
		if (
			...
			context.addIssue({
				code: 'custom',
				message:
					'unavailable descriptors require binary or unavailableReason terminal availability',
				path: ['unavailableReason'],
			});
		}
		if (descriptor.isBinary && descriptor.unavailableReason !== null) {
			context.addIssue({
				code: 'custom',
				message: 'binary descriptors cannot also declare an unavailableReason',
				path: ['unavailableReason'],
			});
		}
	});
