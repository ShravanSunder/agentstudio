...
const workerDerivationEpoch = 31;
const activeIdentity = reviewIdentity('active', 7, 11);
const candidateIdentity = reviewIdentity('candidate', 8, 21);
...
function makeApplicatorHarness(
	props: {
...
function reviewReset(identity: ReviewMetadataIdentity): BridgeProductReviewMetadataEvent {
	return { ...identity, eventKind: 'review.reset', reason: 'sourceChanged' };
}

function reviewSourceAccepted(identity: ReviewMetadataIdentity): BridgeProductReviewMetadataEvent {
...
function reviewDelta(identity: ReviewMetadataIdentity, toRevision: number): ReviewDeltaEvent {
	return {
		...identity,
		contentSources: [],
...
function reviewPublicationId(sequence: number): string {
	return `00000000-0000-7000-8000-${sequence.toString().padStart(12, '0')}`;
}

function reviewInvalidated(identity: ReviewMetadataIdentity): ReviewInvalidatedEvent {
	return {
		...identity,
...
function reviewMetadataTransport(
	reviewSubscription:
