...
- A final CoreGraphics display-level fallback enumerated two online displays
  and reported zero active displays. Both display captures were produced
  through the runtime `CGDisplayCreateImage` symbol, but both images were fully
  black, matching the earlier `screencapture` full-screen behavior.
