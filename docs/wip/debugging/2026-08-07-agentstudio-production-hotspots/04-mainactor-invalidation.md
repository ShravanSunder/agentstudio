...
A second PID-bound stable sample raised this same presentation lane to 2,998 `OutlineListCoordinator.diffRows` intervals (about 20.3% of one core-equivalent sample intervals), with 2,868 `ForEachList` traversals. At 25 seconds, that is roughly 120 list-diff passes per second, matching display cadence. The sample also showed active Ghostty renderer/Metal work at about 19.3%, while Git status was about 3.0%. The two samples establish that the expensive lane changes with runtime state; they do not establish that every list diff is caused by an atom mutation. A frame-rate revisit/animation invalidation is an equally live lead.

A fresh high-CPU stable sample makes the same shape more pronounced. PID `95537` (`0.0.74`, build `115`) was sampled for 15 seconds; the continuously present main thread contributed 8,359 intervals. `OutlineListCoordinator.diffRows` appeared in 4,642 intervals (55.9%), with 4,525 recursive row-diff intervals and 4,396 `ForEachList` traversals ([sample](/tmp/AgentStudio_2026-08-07_stable-highcpu-primary-95537.sample.txt:45)). The active worker mix in that same sample included 2,628 libgit2 blocking-read intervals (31.4%) and 2,611 aggregate Ghostty renderer intervals (31.2%); those parallel shares can sum above 100% and must not be read as one serialized MainActor stack. The main-thread finding is therefore a strong presentation hotspot, but still not proof of the upstream invalidating mutation.
