...
- Workflow 27 held-comment interaction: `fixed and pass`, folded into the permanent promoted E2E 1/1 with 8 skipped. The first truthful RED showed visible A had readable geometry but no pointer selection because generic `loadingPrevious` presentation applied `inert` and `pointer-events-none`. Same-source refresh presentation no longer borrows explicit-target blocking. During `Update ready`, real Chrome now drags three visible additions rows, opens the endpoint utility, commits a new root draft, commits Save, and keeps the exact body visible after Apply now. Initial/explicit target replacement remains inert in the inverse focused test. Supporting installed-Chrome refresh suite is 30/30; typecheck, format, and scoped type-aware lint pass.
