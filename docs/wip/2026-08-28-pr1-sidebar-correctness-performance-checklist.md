...
- The post-restart isolated `DarwinCompositeFSEventContinuityTests` retry did not execute a test case. Two consecutive PPID-1 `swiftpm-testing-helper` processes retained `.build-agent-1`; five-second samples found only an idle main `CFRunLoop` plus one idle work-queue thread, with no continuity fixture, FSEvents, Git, test-case, or active-task stack. Exact orphan owners were terminated after sampling. This is a host/test-helper startup or teardown blocker, not evidence authorizing product-code or proof-gate changes.
