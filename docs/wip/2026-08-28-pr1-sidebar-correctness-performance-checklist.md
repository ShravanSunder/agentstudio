...
- Exact-head aggregate runs exposed a separate test-clock race: the periodic Git fixture observed a pending sleep and then read/advanced its deadline through separate lock acquisitions, allowing cancellation between them. `TestPushClock` now selects and advances the earliest pending sleep atomically; the fixture retries only on the exact pending-sleep event and uses no wall-clock sleep.
- The final remediation review found two additional source-backed PR1 gaps. Process-owned projected worktree edits were incorrectly excluded with owned remote-ref promotion, and Refresh settled after enqueueing represented worktrees rather than after their recomputation. Projected own events now qualify activity while qualifying owned Git metadata still restarts coverage; promoted authority now creates one captured aggregate recomputation lease, retains remote physical custody after automatic demand contracts, and settles progress from every represented worktree without admitting a second explicit local source.
