...
| Launch / app restart | persisted workspace composition and durable zmx sessions are re-admitted in a new process | app boot → foreground main cohort → visible drawer cohort → inactive/background panes deferred until demand | new registry/manager/recorder; no prior host/surface/undo identity survives; deferred panes remain canonical without renderers | each admitted surface begins paused and reconciles from new-run projection; the continuity fixture leaves its drawer expanded before persistence, then selecting each deferred tab mounts only that tab's visible panes through the steady-state restore path; counters begin at zero | new run marker/PID, foreground-before-drawer ordering, no eager deferred renderers, demand-driven new renderer IDs, durable output continuity |
...
| R14 | recorder/manager/registry are process-run state; canonical/zmx restore only; foreground main and visible drawer cohorts precede demand-driven inactive/background renderer admission | restart produces a new marker and counters, proves deferred panes have no eager renderer, then reveals each tab through the normal command/steady-state restore path and verifies new host/surface identities with durable output | process boundary + runtime proof |
