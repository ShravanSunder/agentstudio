...
| E3 | Fast lane: 5,218 test results whose durations sum to 183,718 s inside a 125 s run; median 44.6 s, p90 52.6 s. CI pins Xcode 26.3, whose Swift Testing has no parallelism cap; Xcode 26.6 has one and is the runner image default. Runner is 3 vCPU / 7 GB; the owner's machine is 16 cores / 64 GB | `lane-swift-runner-starvation.md`; reproduced from the raw log; binary inspection of both Xcode versions |
