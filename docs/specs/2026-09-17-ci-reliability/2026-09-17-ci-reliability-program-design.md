...
| D10 | Fresh metadata stream and stale subscriptions (added 2026-09-18) | A metadata stream opened WITHOUT a resume cursor is the client's statement that it holds no subscription ids: it poisoned its session after an exhausted recovery attempt, or it is a new worker. On such an open the pane metadata coordinator captures the session's subscription ids at install, where the client provably cannot have subscribed yet (the client subscribes only after the opening frame, which is enqueued after install), retires exactly that set through the existing per-subscription retirement instead of replaying them, and lets the client re-open what it still needs with fresh ids. A resume, with or without a sequence gap, replays as before. The worker's fail-closed unknown-id check and the wire contract are unchanged | Native replayed every session snapshot onto every installed stream; a client that had forgotten its ids treated the first replayed frame as fatal and the page wedged ("Update unavailable / Retry"). The registry's `.snapshotRequired` disposition cannot discriminate, because a gapped resume is also `.snapshotRequired`; the cursor can. Capture at install is exact by causality, not by timing | One captured array on the coordinator's active-stream record; one session method that terminates a given set. The client owns re-subscription after a fresh open; native no longer replays for it | A supported client opens fresh while still holding subscription ids (none does today; the worker allocates fresh ids after any fresh open) |
...
| BridgeWeb stream recovery (`disconnected` case) | Production: native no longer replays stale subscriptions onto a fresh stream (D10); worker-level red case and native replay suite; the E2E keeps only the wiring claim | 1 |
