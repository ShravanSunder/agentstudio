...
| S25 | “make that into an approval later ... notification later ... for now we can just prepare it in Bridge” (transcription normalized) | Agent file.open prepares/retains the file without automatically entering fullscreen or taking focus. Preparation and visible arrival are distinct outcomes. |
| S26 | “redo all agent session and notifications later with session screen” (transcription normalized) | Sessions-screen, notification, popover and approval work is deferred; this PR does not build or prewire it. |
