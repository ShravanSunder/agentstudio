...
Date: 2026-07-10
Status: accepted normative companion contract after adversarial review
...
`appTick` and `directMainActorHostCall(GhosttyHostCallKind)` are host-scoped MainActor
origins. Host scope establishes affinity and generation, not user authority.
Only `.userKey`/`.userMouseButton` may carry one initially unbound, one-use
`UserIntentGate`; the first eligible descriptor binds and consumes it.
`foreignVendorThread` means no live host origin scope, even
if the callback happens to run on the main thread.
...
| --- | --- | --- |
| `HC` host command | only `directMainActorHostCall(.userKey/.userMouseButton)` whose unconsumed event gate can atomically claim `workspaceCommand`; `hostCompletedTrue` only after the targeted owner synchronously commits the canonical command effect; app tick, programmatic/lifecycle, consumed/missing gate, and foreign callbacks return `hostDeclinedFalse` with no delayed effect | one exact `TerminalCommandRequest` to the named owner; no global bus or replay; ordered in the calling host scope. Destructive close commits closing synchronously but defers physical surface free to the exact post-host-call cleanup lane after the C stack returns |
...
| `SA` snapshot plus deduped fact | host-scoped only; `hostCompletedTrue` after the MainActor generation-bound current snapshot commits; foreign/stale/invalid/oversized input returns `false` with no later projection | one latest bounded snapshot plus a deduped semantic fact only for the named non-source consumer; snapshot sync, not history replay; equal intermediate values contract |
...
| `EF` exact fact | selected tags are host-scoped only; `exactRequestAcceptedTrue` only after `TerminalFactOwner` synchronously commits generation, sequence, and the fact/current snapshot or an explicit non-current `TerminalFactGap`; stale/invalid/closed returns `false`; a future foreign producer requires spec/source reconvergence | exact per-surface/runtime order; bounded history plus non-evictable gap state; cannot share capacity with presentation samples; transport is downstream of the authoritative commit |
| `NS` notification request | selected tags are host-scoped only; `exactRequestAcceptedTrue` only after bounded payload validation, abuse-policy admission, generation validation, and exact inbox admission; deliberate policy rejection returns `false`; a future foreign producer requires spec/source reconvergence | targeted notification/inbox owner; no raw terminal string in routine replay or telemetry; inbox owns durable history and dedupe |
| `PR` privileged request | only `directMainActorHostCall(.userKey/.userMouseButton)` whose unconsumed event gate can atomically claim `clipboardWrite`; `hostCompletedTrue` only after the named MainActor owner validates policy and performs the effect; app tick, programmatic/lifecycle, consumed/missing gate, and foreign callbacks return `false` with no delayed effect | targeted owner only; no global bus/replay; resulting low-rate audit fact is separate and content-free |
| `PC` policy-consumed privileged request | a recognized tag always returns `policyConsumedTrue` after a synchronous allow/deny decision because vendor `false` would invoke a fallback; only an unconsumed event gate nested in a current `.userKey` or `.userMouseButton` call may claim `openURL` and perform one effect; invalid/stale/foreign/missing/consumed input is deliberately denied and consumed | targeted MainActor policy owner only; denial schedules no task and suppresses vendor fallback; no global bus/replay or raw-value telemetry |
...
and `EF`/`NS` cannot evict an accepted exact request. `EF` capacity exhaustion
commits explicit gap/non-current state rather than returning a misleading normal
delivery; such a gap is a correctness and performance acceptance failure.
...
Selected-vendor source also fixes the `SA` origin: IO-originated title/PWD
changes are queued into `Surface.handleMessage` and dispatched on the Ghostty
app thread, while binding/lifecycle title emissions are nested in a direct host
call. No selected `SA` row is authorized from `foreignVendorThread`. The `HC`
and `PR` restrictions are stronger still: app-thread affinity never substitutes
for an unconsumed physical-event user-intent gate.
...
| `GHOSTTY_ACTION_NEW_TAB` | P+C | `HC` | `TerminalWorkspaceCommandSink` -> `PaneTabViewController` resolver | low | no payload |
| `GHOSTTY_ACTION_CLOSE_TAB` | P+C | `HC` | `TerminalWorkspaceCommandSink` -> `PaneTabViewController` resolver | low | bounded close-mode enum |
| `GHOSTTY_ACTION_NEW_SPLIT` | P+C | `HC` | `TerminalWorkspaceCommandSink` -> `PaneTabViewController` resolver | low | bounded direction enum |
...
| `GHOSTTY_ACTION_MOVE_TAB` | P+C | `HC` | `TerminalWorkspaceCommandSink` -> `PaneTabViewController` resolver | low | bounded signed delta |
| `GHOSTTY_ACTION_GOTO_TAB` | P+C | `HC` | `TerminalWorkspaceCommandSink` -> `PaneTabViewController` resolver | low | bounded target enum |
| `GHOSTTY_ACTION_GOTO_SPLIT` | P+C | `HC` | `TerminalWorkspaceCommandSink` -> `PaneTabViewController` resolver | low | bounded direction enum |
| `GHOSTTY_ACTION_GOTO_WINDOW` | P+C | `HS` | AgentStudio window lifecycle suppression | low | bounded target enum |
| `GHOSTTY_ACTION_RESIZE_SPLIT` | P+C | `HC` | `TerminalWorkspaceCommandSink` -> `PaneTabViewController` resolver | interactive | bounded direction and amount |
| `GHOSTTY_ACTION_EQUALIZE_SPLITS` | P+C | `HC` | `TerminalWorkspaceCommandSink` -> `PaneTabViewController` resolver | low | no payload |
| `GHOSTTY_ACTION_TOGGLE_SPLIT_ZOOM` | P+C | `HC` | `TerminalWorkspaceCommandSink` -> `PaneTabViewController` resolver | low | no payload |
...
first-purpose claim plus missing/consumed/chained user intent, Boolean return, stale
generation, saturation, content/export disposition, and the absence of a
delayed host/product effect after `false`; the row-declared
...
Destructive `HC` proof verifies the canonical closing transition commits before
`true`, physical surface free does not occur reentrantly inside the action/C
call, and the exact generation-bound cleanup runs once when the outer host scope
returns.
