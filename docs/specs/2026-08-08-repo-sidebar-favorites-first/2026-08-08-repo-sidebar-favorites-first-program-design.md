...
Add a small RepoExplorer-owned section descriptor with stable identity (`favorites`, `repositories`, `tabs`), resolved groups, and loading repositories. Favorites and Repositories descriptors exist when either collection is non-empty; By Tab always emits its `Tabs` descriptor so the mode retains an explicit heading even when its collections are empty. Its flattening order is section header, resolved group/expanded-leaf runs, then the existing `Scanning…` label and loading rows. Section identity is distinct from semantic group identity. Existing `repo:`, `pane-repo:`, and `tab:` group IDs remain unchanged so disclosure memory, menus, row resolution, and destination identity survive reordering.
