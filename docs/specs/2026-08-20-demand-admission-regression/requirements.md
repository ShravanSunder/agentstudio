...
- The product owner authorizes this work to remove the `v0.0.89` and `v0.0.90` performance regressions while preserving their intended sidebar activity and pull-request-loading outcomes. The owner requires a real-size workspace with no terminal command/output or agent execution inside the isolated debug fixture to have settled process CPU p99 `<10%`, and requires each ordinary sidebar search/clear, sidebar grouping switch, sidebar hide/show, and tab switch population to have process CPU p95 `<20%`. The proof workspace must watch the complete local `opensource` and `project-dev` roots and must contain five tabs and twenty pane models. Unrelated resident host processes are not debug-fixture agents merely because their executable names contain an agent product name; their measured CPU and any actual interfering work remain governed by the host-validity envelope. Agents and terminals must retain the remaining machine budget. `v0.0.88` remains a historical regression control, not the acceptance target.
...
U1 needs `v0.0.89` and `v0.0.90` outcomes without their MainActor saturation. With the complete `opensource` and `project-dev` watched-root topology loaded into an isolated debug workspace containing five tabs and twenty pane models, and with no terminal command/output or agent execution inside that fixture, settled process CPU p99 must be `<10%` both without mounted terminals and with mounted but quiescent terminals that produce no commands or output. Process CPU p95 for ordinary sidebar search/clear, sidebar grouping switching, sidebar hide/show, and tab switching must be `<20%` in each independently measured action population. The proof must account for mounted PTYs and zmx sessions before, during, and after each population so fixture construction or cleanup cannot consume terminal capacity silently. UI state, read-model binding, visible UI updates, focus, layout, and observability work must not consume the compute budget needed by active agents and terminals or produce sustained actor starvation, multi-second terminal round trips, or interaction loss.
...
- The CPU objectives are workload-bound owner targets, not universal hardware promises. Proof must bind them to one recorded real-size workspace on the same hardware, power mode, and build flavor; prove that the complete `opensource` and `project-dev` watched roots and the five-tab/twenty-pane fixture remain represented; record mounted-PTY and zmx-session capacity before, during, and after every population; keep both idle variants and every action class as separate measured populations; and compare the same workload and standard instrumentation configuration before and after the repair. An average or percentile mixed across idle variants, action classes, startup, fixture construction, diagnostic instrumentation, external load, or terminal/agent work inside the debug fixture cannot satisfy the contract.
