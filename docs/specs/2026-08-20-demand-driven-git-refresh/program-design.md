...
The classifier is reused rather than its results becoming a second persisted truth. `RepositoryFactDemandCoordinator` evaluates it off MainActor for source admission. `RepoExplorerProjectionWorker` evaluates the same function off MainActor for presentation. Both receive the same canonical topology, activity/coverage evidence, open-pane associations, reference instant, and policy threshold. Equality at a qualifying-activity or coverage boundary plus sixty days remains non-expired, so every legal transition occurs at the first representable `Date` after equality (`expiration.timeIntervalSinceReferenceDate.nextUp`). A closed warm repository contributes its qualifying-activity expiration, where it becomes inactive when coverage already spans the horizon or unknown when coverage is still maturing. A closed coverage-maturing unknown repository contributes its coverage-start expiration, where it becomes inactive if no newer activity arrived. An open-pane override contributes no time boundary because closing the pane supplies the reclassification input. Unknown evidence without continuous authoritative coverage contributes no deadline. The demand lane owns one reschedulable earliest disposition-change wake, and Repo Explorer folds the same instant into its existing deadline task. Waiting stays off MainActor. Activity, coverage, topology, or pane-association change replaces the pending complete input; stale generations have no authority.
...
The capture is ID-only and uses dedicated association and residency keyed facts rather than composite pane or worktree models. `WorkspacePaneGraphAtom` owns `paneID -> (repoID?, worktreeID?)` association slots plus their membership revision and `paneID -> SessionResidency` slots. Association slots update only when pane membership or that association changes; residency slots update only when pane membership or residency changes. `RepositoryTopologyAtom` owns `worktreeID -> repoID` membership slots plus their membership revision; it updates a slot only when repository/worktree membership changes. These are narrow read interfaces over the existing canonical atoms, not new mutable truth owners. Together they let an active associated pane create the ephemeral open-pane override while preventing CWD, path, name, note, content, drawer placement, or other unrelated changes from invalidating demand capture.

The coordinator observes active tab/window/sidebar presentation state, pane-association membership, repository/worktree membership, and the association and residency slots needed for the complete snapshot. The strict performance-proof classifier consumes the same association-plus-active-residency projection. `FilesystemProjectionIndex` remains a filesystem event/topology projection owner and is not attention authority. The capture does not observe `SidebarVisibleWorktreesRuntimeAtom`, composite `PaneStructuralFacts`, composite `Pane`/`Worktree` values, branch or enrichment facts, filesystem paths, cache dictionaries, search, grouping, viewport rows, row presentation, or every pane's display state. `ForgeActor` retains its existing actor-owned worktree membership and resolves current non-empty branches after receiving demanded worktree IDs; branch changes therefore alter Forge scope without creating a second MainActor branch owner. Membership/topology and active-residency changes may rebuild the compact identity projection; search, grouping, scrolling, row rendering, unrelated pane facts, and unrelated enrichment writes do not rebuild or deliver demand. Content equality runs before any source-owner call.
...
At the current resolved package revision `c8dbd7ef0f344293160b8f7d72d93931328761fd`, `agentstudio-git` is Git-shaped and product-agnostic and exposes separate capabilities:
...
Closed warm transitions at qualifying-activity expiry: directly to inactive when coverage already spans the horizon, otherwise to unknown. Coverage-maturing unknown transitions to inactive only at coverage expiry with no newer activity. Qualifying activity drives unknown/inactive-to-warm; an associated pane supplies only a current-session warm override. Coverage loss drives `unknown`, never inactive. Each transition recomputes and schedules at most one successor through the same earliest-boundary owner. Refresh itself does not change activity. Search, grouping, scrolling, disclosure, and materialization are illegal initiators. A stale classifier or deadline generation publishes nothing.
...
injected clocks + controllable local/child providers
  ...
  -> warm-to-unknown-to-cold and warm-to-cold boundary transitions with exact equality/nextUp semantics;
     unknown closed/sidebar-only keys never inherit visible cadence
...
- qualifying live canonical-ref `OwnEvent` observations restart coverage instead of minting local activity, while the same event still invalidates Git correctness;
- post-restart authority accumulates only for repository keys that receive a current-session checkpoint;
- explicit repository Refresh leaves launcher/application-open recency unchanged;
...
FSEvents replay across downtime plus durable exact attempt/mutation-set owned-fetch provenance remain later full-program targets outside this PR1 correction. Their absence leaves affected activity unknown and therefore uses the bounded background-only local path; it never authorizes inactivity or visible/remote fleet cadence.
...
The current exact package pin `c8dbd7ef0f344293160b8f7d72d93931328761fd` is the PR1 live-attribution baseline. It performs final canonical-ref promotion in process with captured old-target verification while leaving staging, remote child execution, cleanup, and public result contracts unchanged. PR1 treats qualifying live `OwnEvent` ref changes conservatively as unknown coverage. Later full-program work adds durable attempt/mutation-set provenance and downtime replay without restoring the child-promotion path. Rollback must restore the package pin and App attribution behavior together; there is no simultaneous old/new promotion path.
