...
Each cached fact MUST retain source identity, owning entity identity, accepted generation, and freshness/invalidation state sufficient to distinguish fresh, stale-but-usable, unknown, unavailable, and wrong-identity values. Stale-but-usable or unavailable refresh MUST preserve the last facts confirmed for the current identity. Locally inactive presentation MUST retain the last accepted branch and Git/PR chips while adding the specified inactivity status and update control; the inactivity annotation communicates that recurring refresh is suppressed and the accepted facts may be stale. Origin, branch, worktree, or repository replacement MUST reject facts belonging to the old identity.
...
The locally inactive repository header MUST show one compact `memorychip` status icon with tooltip/accessibility copy `Locally inactive` without hiding the last accepted branch or local Git and PR/check chips. Activating that status MUST reveal one compact `Refresh` chip for thirty seconds without moving the row. While refresh is admitted, the header control slot MUST show a compact circular progress indicator with tooltip/accessibility copy `Updating repo`; cached facts remain visible. These states MUST use the existing trailing control slot and MUST NOT add a row or change header height.
...
- A remote fetch failure before canonical-ref promotion preserves current-origin accepted facts and enters only its existing bounded failure backoff. A promotion failure MAY leave some canonical remote refs changed: it MUST remain a failed outcome, invalidate potentially stale same-identity authority even if demand was removed, and MUST NOT advance successful-fetch freshness. While current demand remains, fresh local authority MAY be restored only for the previously accepted current origin, followed by represented-worktree recomputation before failed settlement. Replacement-origin failure MUST NOT authorize previous-origin refs. No rollback or additional recovery system is required.
- Forge failure preserves current-origin accepted facts and enters only its bounded recovery class.
...
- An explicit Refresh settles after its one remote fetch and any authority-valid represented-worktree recomputation; failure follows the promotion distinction above and does not fabricate local activity or fetch success.
