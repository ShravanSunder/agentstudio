...
- **Configuration popover content**: Arrangements and sidebar Sort/Group/Subgroup use `PopoverPanel`, `PopoverPanelSectionHeader`, and `PopoverOptionButtonStyle` / `PopoverOptionSurface`. The shell owns one content inset; option surfaces own explicit typography, padding, and selected/hover/pressed paint. Use `PopoverOptionLabel` for icon-and-name rows. Do not add nested panel padding or private copies of the option style. Hosts retain their placement and command/keyboard owners.
