...
| `WorkspaceSurfaceCoordinator.createViewForContent` | `App/Coordination/WorkspaceSurfaceCoordinator.swift` | Routes `PaneContent` → view creation; webview case at line 101 |
| `WorkspaceSurfaceCoordinator.openWebview(url:)` | `App/Coordination/WorkspaceSurfaceCoordinator.swift` | Creates browser pane with `WebviewState` + tab |
...
`WorkspaceSurfaceCoordinator.createViewForContent` gains a new case:
...
- **Creation**: `WorkspaceSurfaceCoordinator.openDiffViewer()` → `WorkspaceSurfaceCoordinator.createViewForContent(.bridgePanel)` → `BridgePaneController` + `BridgePaneMountView` mounted inside `PaneHostView` → `ViewRegistry.register`
...
- ~~**Pane system integration**~~ → Full create/teardown/persist/restore lifecycle working. `PaneContent.webview`, `WebviewState`, `WorkspaceSurfaceCoordinator.createViewForContent`, `WorkspaceSurfaceCoordinator.openWebview`, `ViewRegistry` — all operational.
