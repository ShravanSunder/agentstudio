...
| How does AgentStudio link `agentstudio-git`, and may I shell out to `git`? | [agentstudio-git](state/agentstudio_git.md#agentstudio-git) | You spawn `git`/`wt`, treat the package as a Ghostty vendor, or duplicate Git policy in TypeScript. | [`Package.swift`](../../Package.swift) revision pin; `LibGit2AgentStudioGitLocalClient` adapters |
...
| [agentstudio-git](state/agentstudio_git.md) | External Git package pin and in-app adapters |
