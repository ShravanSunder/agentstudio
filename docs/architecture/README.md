...
| How do the native titlebar, tab hit testing, tab dragging, and window dragging fit together? | [AppKit + SwiftUI Hybrid Architecture — Native Titlebar And Tab Strip](appkit_swiftui_architecture.md#native-titlebar-and-tab-strip) | `MainWindowController`, `MainToolbarChromeView`, `DraggableTabBarHostingView`, window-rooted toolbar tests |
...
| What may run on MainActor vs off-main? | [Pane Runtime EventBus Design](pane_runtime_eventbus_design.md) | Terminal drain/projector, `EagerDerivedAtom`, `FilesystemProjectionIndex` |
...
| How should expensive derived facts react to product demand without polling or stale publication? | [Demand-Driven Derived-State Refresh](demand_driven_derived_state_refresh.md) | The concrete observer, admission owner, executor, publication path, and keyed read model |
