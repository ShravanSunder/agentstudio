...
| [Pane Runtime EventBus Design](pane_runtime_eventbus_design.md) | EventBus coordination: actor fan-out, boundary actors (filesystem/network), `@concurrent` for per-pane work, multiplexed `@Observable` + event stream, data flow per contract, Swift 6.2 threading model |
