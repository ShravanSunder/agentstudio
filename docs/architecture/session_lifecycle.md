...
|    Session: agentstudio--a1b2c3d4--e5f6a7b8              |
...
```
agentstudio--<project8hex>--<worktree8hex>
            |              |
            |              +-- First 8 hex chars of worktree UUID (lowercase)
            +-- First 8 hex chars of project UUID (lowercase)

Example: agentstudio--a1b2c3d4--e5f6a7b8
Length:  31 characters (fixed)
```

Generated by `TmuxBackend.sessionId(projectId:worktreeId:)`. Deterministic — same project+worktree pair always produces the same session ID.

Validated by `PaneSessionHandle.hasValidId`: must be exactly 31 chars, start with `agentstudio--`, and contain only lowercase hex in both 8-char segments.
...
2. `attachCommand(for:in:)` — returns `tmux -L agentstudio -f ghost.conf new-session -A -s <id> -c <cwd>`
...
Used by `getOrCreatePaneSession(for:in:)`:
...
```json
{
  ...
  "sessions": [
    {
      "sessionId": "agentstudio--a1b2c3d4--e5f6a7b8",
      "projectId": "A1B2C3D4-...",
      "worktreeId": "E5F6A7B8-...",
      "displayName": "main",
...
3. For each saved session, calls `backend.sessionExists()` (`tmux has-session`)
4. Only sessions that are still alive in tmux are added to `entries`
5. Dead sessions are silently skipped
...
3. When the view enters a window (`viewDidMoveToWindow`), if `surfaceId == nil`:
...
This deferred pattern prevents orphaned processes when restoring many tabs.
