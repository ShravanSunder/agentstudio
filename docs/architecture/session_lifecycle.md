...
|    Session: agentstudio--a1b2c3d4--e5f6a7b8--00001111    |
...
```
agentstudio--<project8hex>--<worktree8hex>--<pane8hex>
            |              |                |
            |              |                +-- First 8 hex chars of pane UUID (lowercase)
            |              +-- First 8 hex chars of worktree UUID (lowercase)
            +-- First 8 hex chars of project UUID (lowercase)

Example: agentstudio--a1b2c3d4--e5f6a7b8--00001111
Length:  41 characters (fixed)
```

Generated by `TmuxBackend.sessionId(projectId:worktreeId:paneId:)`. Deterministic — same project+worktree+pane triple always produces the same session ID. The `paneId` is the `containerId` from `AgentStudioTerminalView`, which is persisted through the split tree Codable chain. This ensures split panes get independent tmux sessions.

**Legacy format (31 chars, 2 segments):** `agentstudio--<proj8>--<wt8>`. Accepted by `PaneSessionHandle.hasValidId` for backward compatibility, but old sessions are destroyed during checkpoint restore.

Validated by `PaneSessionHandle.hasValidId`: must start with `agentstudio--`, contain 2 or 3 segments of exactly 8 lowercase hex chars each.
...
`SessionCheckpoint` (version 2, JSON):
...
3. Legacy 2-segment sessions (31-char IDs) are destroyed via `destroySessionById()` and skipped
4. For each 3-segment session, calls `backend.sessionExists()` (`tmux has-session`)
...
| `Models/SessionCheckpoint.swift` | Codable checkpoint model (v2 JSON) |
