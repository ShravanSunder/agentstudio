...
> **Note on `.worktreeBellRang` (post LUNA-361).** The notification inbox feature consumes `GhosttyEvent.bellRang` directly from `PaneRuntimeEventBus` via `NotificationRouter`, not from `AppEventBus.worktreeBellRang`. Whether the existing `AppEventBus.worktreeBellRang` post in `PaneCoordinator` stays as a peer signal for other consumers or is removed is a follow-up decision; the notification inbox does not depend on it.
