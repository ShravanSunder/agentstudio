...
| P9 | **No external automation surface** | Users can't script workspace operations or integrate with external tools; no CLI or socket API to create panes, inject notifications, or manipulate layouts from outside the app | JTBD 3, 8 |
| P10 | **Passive notifications** | Agent completion, failure, or input-needed events aren't surfaced without manually checking each pane; no visual indicators or notification routing | JTBD 6 |
...
### R8: Notification Routing

Agent events (completion, failure, input-needed) are surfaced visually without requiring the user to check each pane. Notification routing uses OSC terminal sequences (OSC 9/99/777) as the agent-agnostic input channel.

- **Pane-level indicator**: Visual ring/badge on the pane that emitted the notification
- **Sidebar badge**: Tab/workspace entry in sidebar highlights with unread count
- **Notification panel**: Consolidated list of pending notifications with pane context (repo, worktree, agent type)
- **Jump-to-unread**: Keyboard shortcut to navigate to the most recent unread notification
- **Notification text in sidebar**: Latest notification snippet visible per tab without expanding
- OSC 9 (basic notify), OSC 99 (kitty notify with id/title), OSC 777 (rxvt notify) supported
- Notifications inherit pane metadata (repo, worktree, agent type) for grouping and filtering
- CLI injection: notifications can also be sent via external API (see R9)
- Read/unread state persists per notification

**Solves**: P6, P10 | **Enables**: JTBD 6

### R9: External API

A CLI and socket API for external tools and scripts to interact with the workspace. Enables automation, integration with CI/CD, and custom workflows without requiring changes to Agent Studio itself.

- **CLI commands**: Create/list/close panes, send notifications, query workspace state, manipulate layouts
- **Socket API**: Unix domain socket for programmatic access with JSON protocol
- **Workspace queries**: List panes by repo/worktree/agent, get pane metadata, read notification state
- **Pane creation**: Spawn terminal panes with pre-set CWD, worktree association, and agent type
- **Notification injection**: Send notifications to specific panes or globally from external processes
- API operations go through the same action pipeline as command bar (Parse → Validate → Execute → Emit)
- Discoverable via `agentstudio --help` or socket introspection

**Solves**: P9 | **Enables**: JTBD 3, 8
...
```
                    ...
                    pane     static   lost     orphan   no       context  fragment parallel no ext   passive
                    explode  groups   context  ephem    compose  switch   context  tooling  automate notify
