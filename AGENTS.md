...
| `WorkspaceCatalogAtom` | workspace metadata, repos, worktrees, watched paths, availability | `Core/State/MainActor/Atoms/WorkspaceCatalogAtom.swift` |
| `WorkspaceGraphAtom` | panes, tabs, layouts, drawers, undo snapshots | `Core/State/MainActor/Atoms/WorkspaceGraphAtom.swift` |
| `WorkspaceInteractionAtom` | dragging/drop/frame/split interaction state | `Core/State/MainActor/Atoms/WorkspaceInteractionAtom.swift` |
...
ForgeActor      ──► .pullRequestCountsChanged ──────────────────────────┘      │
                                              ...
                                              ┌────────────────────────┼──────────────────────┐
                                              ▼                        ▼                      ▼
                                       WorkspaceCatalogAtom    RepoCacheAtom         TopologyEffectHandler
                                       WorkspaceGraphAtom      (enrichment)          (PaneCoordinator)
                                              │                        │              orphan panes +
                                              └────────────┬───────────┘              sync FS roots
                                                           ▼
...
│   │   ├── State/
│   │   │   └── MainActor/
│   │   │       ├── Atoms/            # WorkspaceCatalogAtom, WorkspaceGraphAtom, WorkspaceInteractionAtom, ...
...
| `WorkspaceCatalogAtom` | `Core/State/MainActor/Atoms/` | Workspace metadata, repos, worktrees, watched paths, availability |
| `WorkspaceGraphAtom` | `Core/State/MainActor/Atoms/` | Pane/tab/layout graph, drawers, undo snapshots |
| `WorkspaceInteractionAtom` | `Core/State/MainActor/Atoms/` | Dragging/drop/frame/split interaction state |
